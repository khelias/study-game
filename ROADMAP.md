...
- **Content-pack depth is uneven.** The registry now has 19 skills and 22 packs, but several packs are still progression specs rather than deep content banks: arithmetic snake packs often have 2-3 DSL spec items, and `language.spatial_sentences.scene_pack` has 8 scenes. `math.balance_equations.core`, `math.time_reading.core`, and `math.addition_memory.core` have been expanded to level-stage packs, but the remaining shallow procedural packs still need an explicit spec-coverage audit rule.
