...
keelepackidel puudub explicit difficulty/learning-outcome signaal.
...
`math_snake` DSL-spec packid ja `math.unit_conversions.core` on nüüd
auditiloogikas eraldi kaetud. Kõige praktilisem järgmine slice on
`language.spatial_sentences.scene_pack`:

- lisada scene'idele difficulty või level/focus metadata;
- katta asukohamõisted õppesisu signaalidega;
- uuendada generatori ja packi teste, et valik kasutab staged metadata't;
- pärast seda jätkata `language.syllabification.*` ja `language.vocabulary.*`
  tagidega.
