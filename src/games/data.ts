...
export const WORD_DB: Record<number, WordObject[]> = BASE_WORDS.reduce<
  Record<number, WordObject[]>
>((acc, item) => {
...
export const WORD_DB_EN: Record<number, WordObject[]> = BASE_WORDS_EN.reduce<
  Record<number, WordObject[]>
>((acc, item) => {
  const len = item.w.length;
  if (!acc[len]) acc[len] = [];
